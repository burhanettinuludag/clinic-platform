...
import { Moon, Clock, Eye, ArrowLeft, BookOpen, User, Calendar, ChevronRight } from 'lucide-react';
import { useSleepArticle, useSleepArticles } from '@/hooks/useSleepData';
...
function renderMarkdown(content: string) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith('> ')) {
      ...
      elements.push(
        <blockquote key={i} className="border-l-4 border-teal-500 bg-teal-50 px-4 py-3 my-4 rounded-r-lg text-sm text-teal-800">
...
export default function SleepArticlePage() {
  const params = useParams();
  const locale = (params?.locale as string) || 'tr';
  const slug = params?.slug as string;
  const t = useTranslations('sleep');

  const { data: article, isLoading } = useSleepArticle(slug);
  const { data: relatedArticles = [] } = useSleepArticles(
    article?.related_disease
      ? { related_disease: article.related_disease }
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center">
        <div className="text-center">
          ...
          <h2 className="text-xl font-semibold text-gray-700 mb-2">{t('article.notFound')}</h2>
          <Link href={`/${locale}/sleep`} className="text-teal-600 hover:underline">
            {t('article.backToSleep')}
          </Link>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className={`bg-gradient-to-br from-indigo-900 via-indigo-800 to-purple-900 text-white ${coverUrl ? '-mt-24 relative z-10' : ''}`}>
        <div className="max-w-3xl mx-auto px-4 py-12">
          <Link
            href={`/${locale}/sleep`}
            className="inline-flex items-center gap-1.5 text-indigo-300 hover:text-white transition-colors text-sm mb-6"
          >
          ...
          {article.subtitle && (
            <p className="text-lg text-indigo-200">{article.subtitle}</p>
          )}

          <div className="flex items-center gap-4 mt-6 text-sm text-indigo-300 flex-wrap">
      ...
      {otherArticles.length > 0 && (
        <section className="bg-gray-50 border-t border-gray-200">
          <div className="max-w-3xl mx-auto px-4 py-12">
            <h3 className="text-xl font-bold text-gray-900 mb-6">{t('article.related')}</h3>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {otherArticles.map(a => (
                <Link
                  key={a.id}
                  href={`/${locale}/sleep/${a.slug}`}
                  className="bg-white rounded-xl p-4 border border-gray-200 hover:border-teal-300 hover:shadow-sm transition-all group"
                >
                  <h4 className="font-semibold text-sm text-gray-900 group-hover:text-teal-700 transition-colors line-clamp-2 mb-2">
                    {a.title}
                  </h4>
