...
export default function ParkinsonEducationPage() {
  const locale = useLocale();
  const tr = (trText: string, enText: string) => (locale === 'tr' ? trText : enText);

  const { data: educationItems, isLoading: loadingItems, isError: errorItems } = useEducationItems({ disease_module: 'parkinson' });
  const { data: quizzes, isLoading: loadingQuizzes, isError: errorQuizzes } = useQuizzes({ disease_module: 'parkinson' });
  ...
  const modules = useMemo<CategoryGroup[]>(() => {
    ...
    return MODULE_ORDER.map((slug) => {
      ...
      return {
        slug,
        name: meta ? (locale === 'tr' ? meta.label_tr : meta.label_en) : slug,
        items,
        quiz,
      };
    }).filter((m) => m.items.length > 0);
  }, [educationItems, quizzes, locale]);

  if (loadingItems || loadingQuizzes) {
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <div className="animate-pulse text-gray-500">{tr('Yükleniyor...', 'Loading...')}</div>
      </div>
    );
  }
    ...
    return (
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center gap-4 mb-6">
          ...
          <h1 className="text-xl font-bold text-gray-900">{tr('Parkinson Eğitim Seti', 'Parkinson Education')}</h1>
        </div>
        <div className="text-center py-12 bg-white rounded-xl border border-red-200">
          ...
          <p className="text-red-600 font-medium">{tr('Eğitim içerikleri yüklenemedi.', 'Failed to load education content.')}</p>
          <button
            onClick={() => window.location.reload()}
            className="mt-4 px-6 py-2 bg-teal-600 text-white rounded-lg hover:bg-teal-700 transition text-sm"
          >
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          <h1 className="text-xl font-bold text-gray-900">{tr('Parkinson Eğitim Seti', 'Parkinson Education')}</h1>
          <p className="text-sm text-gray-500">{tr('5 modül - aşamalı öğrenme programı', '5 modules - progressive learning program')}</p>
        </div>
      </div>

      {/* Progress Overview */}
      <div className="bg-gradient-to-r from-teal-500 to-emerald-600 rounded-xl p-6 text-white mb-6">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            <BookOpen className="w-8 h-8" />
            <div>
              <div className="text-sm opacity-90">{tr('Genel İlerleme', 'Overall Progress')}</div>
              <div className="text-2xl font-bold">{completedCount}/{totalCount} {tr('Kart', 'Cards')}</div>
            </div>
          </div>
          ...
          {modules.map((mod, idx) => {
            ...
            return (
              ...
              >
                ...
                <span className="text-[10px] font-medium text-gray-600 leading-tight">{idx + 1}. {tr('Modül', 'Module')}</span>
            ...
            <div key={mod.slug} id={`module-${mod.slug}`} className="scroll-mt-4">
              ...
              >
                ...
                <div className="flex-1 text-left">
                  <div className="text-xs opacity-80">{tr('Modül', 'Module')} {moduleIdx + 1}</div>
              ...
              {isExpanded && (
                <div className="mt-2 space-y-2 pl-2">
                  {mod.items.map((item, index) => (
                    <EducationCard key={item.id} item={item} index={index + 1} onClick={() => setSelectedItem(item)} />
                  ))}
                  {mod.quiz && (
                    <QuizCard quiz={mod.quiz} onStart={() => setActiveQuizSlug(mod.quiz!.slug)} allItemsDone={allDone} locale={locale} />
                  )}
                </div>
              )}
            </div>
      ...
      {educationItems && educationItems.length === 0 && (
        <div className="text-center py-12 bg-white rounded-xl border border-gray-200">
          ...
          <p className="text-gray-500">{tr('Henüz eğitim içeriği eklenmemiş.', 'No education content yet.')}</p>
          <p className="text-xs text-gray-400 mt-2">{tr('Parkinson eğitim içerikleri yakında eklenecektir.', 'Parkinson education content will be added soon.')}</p>
        </div>
      )}

      {selectedItem && <EducationModal item={selectedItem} onClose={() => setSelectedItem(null)} locale={locale} />}
      {activeQuizSlug && <QuizModal slug={activeQuizSlug} onClose={() => setActiveQuizSlug(null)} locale={locale} />}
    </div>
  );
}
    ...
    >
      <div className="flex items-center gap-3">
        <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 text-sm ${
          isCompleted ? 'bg-green-100 text-green-600' : inProgress ? 'bg-teal-100 text-teal-600' : 'bg-gray-100 text-gray-500'
        }`}>
      ...
      {inProgress && item.progress && (
        <div className="mt-2 pt-2 border-t border-teal-200">
          <div className="h-1.5 bg-teal-100 rounded-full overflow-hidden">
            <div className="h-full bg-teal-500 rounded-full" style={{ width: `${item.progress.progress_percent}%` }} />
          </div>
        </div>
      )}
    </button>
...
function QuizCard({ quiz, onStart, allItemsDone, locale }: { quiz: EducationQuiz; onStart: () => void; allItemsDone: boolean; locale: string }) {
  const tr = (trText: string, enText: string) => (locale === 'tr' ? trText : enText);
  ...
  return (
    <div className={`rounded-xl border-2 p-4 transition-all ${
      passed ? 'border-green-300 bg-green-50' : attempted ? 'border-yellow-300 bg-yellow-50' : 'border-teal-200 bg-teal-50/50'
    }`}>
      <div className="flex items-center gap-3">
        <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 ${
          passed ? 'bg-green-200 text-green-700' : 'bg-teal-200 text-teal-700'
        }`}>
        ...
        <div className="flex-1">
          ...
          <div className="flex items-center gap-3 mt-1 text-xs">
            <span className="text-gray-500">{quiz.question_count} {tr('soru', 'questions')}</span>
            <span className="text-teal-600 font-medium">+{quiz.points_reward} {tr('puan', 'points')}</span>
            {quiz.best_attempt && (
              <span className={`font-medium ${passed ? 'text-green-600' : 'text-yellow-600'}`}>
                ...
                {passed ? tr(' Geçti', ' Passed') : tr(' Geçemedi', ' Failed')}
              </span>
            )}
          </div>
        </div>
        ...
        >
          {passed ? <><RotateCcw className="w-3 h-3" /> {tr('Tekrar', 'Retry')}</> : attempted ? tr('Tekrar Dene', 'Retry') : tr('Quiz Başlat', 'Start Quiz')}
        </button>
      </div>
      {!allItemsDone && !attempted && (
        <p className="text-[10px] text-gray-400 mt-2 text-center">{tr('Quiz\'i açmak için tüm kartları tamamlayın', 'Complete all cards to unlock quiz')}</p>
      )}
    </div>
  );
}
...
function EducationModal({ item, onClose, locale }: { item: EducationItem; onClose: () => void; locale: string }) {
  const tr = (trText: string, enText: string) => (locale === 'tr' ? trText : enText);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-hidden flex flex-col">
        <div className="flex items-center justify-between p-4 border-b bg-gradient-to-r from-teal-50 to-emerald-50">
          <div>
            <h2 className="text-lg font-semibold text-gray-900">{item.title}</h2>
            <div className="flex items-center gap-2 mt-1 text-sm text-gray-500">
              <Clock className="w-4 h-4" />
              <span>{item.estimated_duration_minutes} {tr('dakika', 'minutes')}</span>
              {isCompleted && (
                <span className="flex items-center gap-1 text-green-600 font-medium">
                  <CheckCircle2 className="w-4 h-4" /> {tr('Tamamlandı', 'Completed')}
                </span>
              )}
            </div>
          </div>
        ...
        <div className="p-4 border-t bg-gray-50">
          {isCompleted ? (
            <div className="flex items-center justify-center gap-2 py-2 text-green-600">
              ...
              <span className="font-medium">{tr('Bu içeriği tamamladınız', 'You completed this content')}</span>
            </div>
          ) : isCompleting ? (
            <div className="flex items-center justify-center gap-2 py-2 text-green-600">
              <CheckCircle2 className="w-5 h-5" />
              <span className="font-medium">{tr('Tamamlandı!', 'Completed!')}</span>
            </div>
          ) : !hasStarted ? (
            <button onClick={handleStart} disabled={startEducation.isPending} className="w-full py-3 bg-teal-600 text-white font-medium rounded-lg hover:bg-teal-700 disabled:opacity-50 transition">
              {startEducation.isPending ? tr('Başlatılıyor...', 'Starting...') : tr('Okumaya Başla', 'Start Reading')}
            </button>
          ) : (
            <button onClick={handleComplete} disabled={completeEducation.isPending} className="w-full py-3 bg-green-600 text-white font-medium rounded-lg hover:bg-green-700 disabled:opacity-50 transition">
              {completeEducation.isPending ? tr('Tamamlanıyor...', 'Completing...') : tr('Tamamladım', 'Mark Complete')}
            </button>
          )}
        </div>
      </div>
...
function QuizModal({ slug, onClose, locale }: { slug: string; onClose: () => void; locale: string }) {
  const tr = (trText: string, enText: string) => (locale === 'tr' ? trText : enText);
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div className="relative bg-white rounded-2xl p-8 text-center">
          <div className="animate-pulse text-gray-500">{tr('Quiz yükleniyor...', 'Loading quiz...')}</div>
        </div>
      </div>
    );
  }
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div className="relative bg-white rounded-2xl max-w-md w-full p-8 text-center">
          ...
          <h2 className="text-2xl font-bold text-gray-900 mb-2">
            {result.passed ? tr('Tebrikler!', 'Congratulations!') : tr('Tekrar Deneyin', 'Try Again')}
          </h2>
          <p className="text-gray-600 mb-4">
            {result.score}/{result.total} {tr('doğru', 'correct')} - %{percent}
          </p>
          {result.passed && (
            <div className="flex items-center justify-center gap-2 text-teal-600 mb-4">
              <Award className="w-5 h-5" />
              <span className="font-medium">+{quiz.points_reward} {tr('puan kazandınız!', 'points earned!')}</span>
            </div>
          )}
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      ...
                      {!isCorrect && correctIdx >= 0 && (
                        <p className="text-green-700 mt-1 text-xs">{tr('Doğru cevap:', 'Correct answer:')} {q.options[correctIdx].text}</p>
                      )}
            ...
            {questions.map((q, idx) => (
              <button
                ...
                className={`h-2 flex-1 rounded-full transition-all ${
                  idx === currentQ ? 'bg-teal-600' : answers.has(q.id) ? 'bg-teal-300' : 'bg-gray-200'
                }`}
              />
            ))}
          </div>
          <div className="text-xs text-gray-500 mt-1">{tr('Soru', 'Question')} {currentQ + 1}/{totalQuestions}</div>
        </div>
              ...
              {question.options.map((option, idx) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <span className={`text-sm ${selectedOption === idx ? 'text-teal-900 font-medium' : 'text-gray-700'}`}>
                      {option.text}
                    </span>
                  </div>
                </button>
        ...
        <div className="p-4 border-t bg-gray-50 flex items-center justify-between">
          ...
          {currentQ < totalQuestions - 1 ? (
            <button
              ...
              className="px-4 py-2 text-sm bg-teal-600 text-white rounded-lg hover:bg-teal-700 disabled:opacity-30 transition"
            >
              {tr('Sonraki', 'Next')}
            </button>
          ) : (
            ...
            >
              {submitQuiz.isPending ? tr('Gönderiliyor...', 'Submitting...') : tr('Tamamla', 'Complete')}
            </button>
          )}
        </div>
      </div>
