...
const MODULE_META: Record<string, { icon: typeof BookOpen; color: string; gradient: string; label: string }> = {
  'dementia-basics': {
    ...
    label: 'Demansi Taniyalim',
  },
  'dementia-types': {
    icon: Layers,
    color: 'blue',
    gradient: 'from-blue-500 to-indigo-600',
    label: 'Demans Turleri',
  },
  'dementia-signs': {
    icon: Eye,
    color: 'amber',
    gradient: 'from-amber-500 to-orange-600',
    label: 'Erken Belirtiler',
  },
  'dementia-diagnosis': {
    icon: Stethoscope,
    color: 'purple',
    gradient: 'from-purple-500 to-violet-600',
    label: 'Tani ve Degerlendirme',
  },
  'dementia-care': {
    icon: Heart,
    color: 'rose',
    gradient: 'from-rose-500 to-pink-600',
    label: 'Bakim ve Destek',
  },
};
...
export default function DementiaEducationPage() {
  const { data: educationItems, isLoading: loadingItems, isError: errorItems } = useEducationItems({ disease_module: 'dementia' });
  const { data: quizzes, isLoading: loadingQuizzes, isError: errorQuizzes } = useQuizzes({ disease_module: 'dementia' });
    ...
    return (
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center gap-4 mb-6">
          <Link href="/patient/dementia" className="p-2 hover:bg-gray-100 rounded-lg transition">
            <ArrowLeft className="w-5 h-5 text-gray-600" />
          </Link>
          <h1 className="text-xl font-bold text-gray-900">Demans Egitim Seti</h1>
        </div>
